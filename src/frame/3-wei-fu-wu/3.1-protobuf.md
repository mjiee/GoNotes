...
Protobuf需要先在<mark style="background-color:green;"> .proto <mark>文件中预定义结构化数据, 然后通过<mark style="background-color:green;"> protoc <mark>工具将<mark style="background-color:green;"> .proto <mark>文件转换为对应语言代码。

protoc工具安装, 参见官网: [Protocol Buffers](https://developers.google.com/protocol-buffer)

```shell
$ protoc --version  # 查看版本
$ protoc --proto_path=IMPORT_PATH --<lang>_out=DST_DIR path/file.proto  # 转换数据结构文件
# --proto_path=xx/. 为import导入文件路径; 
--<lang>_out指定语言, 插件和输出路径, 如: --go_out=yy/.
```
...
option go_package="utils";  // 定义go中的包名, 默认使用package值
